...
export default function Home() {
  ...
  const getNext3Days = () => {
    ...
    const nextDays = Object.entries(days)
      ...
      .slice(0, 3)
      .map(([_, values]) => values[4]); // mid-day forecast
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-900 p-4 flex">
        {/* Sidebar */}
        <aside className="w-1/4 bg-gray-800 p-4 rounded shadow-lg flex flex-col justify-between">
          {current && (
            <>
              ...
              <div className="text-center text-sm mt-4">
                <p>{new Date(current.dt_txt).toDateString()}</p>
                <p className="font-semibold mt-1">{weather?.city.name}</p>
              </div>
            </>
          )}
        </aside>
        ...
        <section className="flex-1 ml-4 flex flex-col gap-4">
          {/* Top: Next 3 Days */}
